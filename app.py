...
@app.route('/book')
def public_booking():
    """Public booking page — no login needed. Shows price list, sample albums, booking form."""
    return render_template('index.html', mode='public_booking')
...
def db_stats():
    conn = get_db()
    stats = {}
    tables = ['bookings','clients','payments','galleries','gallery_photos','reminders','inquiries']
    for t in tables:
        try:
            stats[t] = conn.execute(f"SELECT COUNT(*) FROM {t}").fetchone()[0]
        except:
            stats[t] = 0
    stats['total_revenue']      = conn.execute("SELECT COALESCE(SUM(paid),0) FROM payments").fetchone()[0]
    stats['pending_amount']     = conn.execute("SELECT COALESCE(SUM(amount-paid),0) FROM payments WHERE status!='Paid'").fetchone()[0]
    stats['confirmed_bookings'] = conn.execute("SELECT COUNT(*) FROM bookings WHERE status='Confirmed'").fetchone()[0]
    stats['recent_bookings']    = [dict(r) for r in conn.execute("SELECT client,type,date,package,amount,status,created_at FROM bookings ORDER BY created_at DESC LIMIT 5").fetchall()]
    stats['recent_payments']    = [dict(r) for r in conn.execute("SELECT client,session,amount,paid,status,created_at FROM payments ORDER BY created_at DESC LIMIT 5").fetchall()]
    stats['recent_inquiries']   = [dict(r) for r in conn.execute("SELECT name,phone,event_type,status,created_at FROM inquiries ORDER BY created_at DESC LIMIT 5").fetchall()]
    conn.close()
...
def db_table(table_name):
    allowed = ['bookings','clients','payments','galleries','gallery_photos','reminders','inquiries','customer_tokens']
...
def db_export():
    ...
    table = request.args.get('table','bookings')
    allowed = ['bookings','clients','payments','galleries','reminders','inquiries']
    ...
    return Response(output.getvalue(), mimetype='text/csv',
        headers={'Content-Disposition': f'attachment; filename=vip_{table}_{datetime.now().strftime("%Y%m%d")}.csv'})
